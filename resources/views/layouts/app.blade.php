    ...
    <div class="fixed-top">
        <nav id="nav" class="navbar navbar-expand-lg navbar-light bg-white">
            <div class="container-xl d-flex justify-content-between align-items-center">
                ...
                <a href="{{ url('/apropos') }}" class="text-success"><u>A Propos</u><i
                ...
                <div>
                    ...
                    <div class="collapse navbar-collapse" id="navbarSupportedContent">
                        <ul class="navbar-nav me-auto mb-2 mb-lg-0">
                            @auth
                                <li class="nav-item">
                                    <a class="nav-link active" href="{{ route('annonces.create') }}"><u>Déposer une
                                            annonce</u><i class="bi bi-plus-circle"></i></a>
                                </li>
                                <li class="nav-item">
                                    <a class="nav-link" href="{{ route('annonces.index') }}"><u>Rechercher</u><i
                                            class="bi bi-search"></i></a>
                                </li>
                                <li class="nav-item">
                                    <a href="{{ route('annonces.index') }}" class="nav-link"><u>Voir les annonces</u><i
                                            class="bi bi-eye"></i></a>
                                </li>
                                @if(auth()->user()->is_admin === 1)
                                    <li class="nav-item">
                                        <a class="nav-link" href="{{ route("categories.create") }}"><u>Créer une
                                                catégorie</u><i class="bi bi-plus-circle-fill"></i></a>
                                    </li>
                                @endif
                                <li class="nav-item">
                                    <a class="nav-link" href="{{ route("categories.index") }}"><u>Voir les
                                            catégories</u><i class="bi bi-eye-fill"></i></a>
                                </li>
                            @endauth
                            @guest
                                @if (Route::has('login'))
                                    <li class="nav-item">
                                        <a class="nav-link"
                                           href="{{ route('login') }}"><u>{{ __('Connexion') }}</u></a>
                                    </li>
                                @endif
                                @if (Route::has('register'))
                                    <li class="nav-item">
                                        <a class="nav-link"
                                           href="{{ route('register') }}"><u>{{ __('Inscription') }}</u></a>
                                    </li>
                                @endif
                            @else
                                    ...
                                    <div class="dropdown-menu dropdown-menu-end" aria-labelledby="navbarDropdown">
                                        ...
                                        @if(auth()->user()->is_admin === 1)
                                            <a class="dropdown-item" href="{{ route('users.index') }}">
                                                <u>Voir les utilisateurs</u><i class="bi bi-person-circle"></i>
                                            </a>
                                        @endif
